...
impl Bookmark {
    ...
    pub fn mark(&self) -> Result<(), crate::Error> {
        writable_tx(|tx| Ok(self.mark_with(tx)))
    }
    ...
    pub fn unmark_with(&self, tx: &Tx<'_>) {
        let operation = match self.ty {
            BookmarkType::Reference => MergeOperation::Increment(-1),
